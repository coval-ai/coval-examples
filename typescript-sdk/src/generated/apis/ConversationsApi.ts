...
export interface ConversationsApiInterface {
     ...
     * @param {'metric_outputs' | 'metric_values'} [include] Set to &#x60;metric_outputs&#x60; to embed full outputs for &#x60;metric_id&#x60; on every conversation in the returned page, or &#x60;metric_values&#x60; to embed every metric\&#39;s value inline on each conversation (keyset-paginated; &#x60;page_size&#x60; capped at 100). Omitted by default to keep list payloads small. 
    ...
    /**
     * Attach an audio recording, or add metadata keys, to a conversation previously submitted via `POST /v1/conversations:submit`.  **When to attach audio**  Use this endpoint when your recording URL is not available at the moment the call ends (for example, Twilio Programmable Voice recordings that are only fetchable after an asynchronous job completes ~60 seconds later). Submit the transcript immediately at call end to get a `conversation_id` for trace correlation, then PATCH the audio when the recording URL is ready. Attaching audio triggers audio-dependent metrics as a second wave.  **When to add metadata**  Use this endpoint when a value you want to filter or segment on is only known after the call was submitted (for example, an outcome produced by post-call analysis). Once added, the key is immediately available as a filter on the Conversations page and in the `filter` parameter of `GET /v1/conversations`.  **Two-wave metric timing**  - Text-only metrics fire immediately after `POST /v1/conversations:submit`   and deliver their results via your configured webhook. - Audio-dependent metrics fire after an audio PATCH and deliver a separate,   later webhook. Configure your consumer to expect both waves.  Adding metadata does **not** run or re-run any metric. If a metric depends on a key you added, rerun it with `POST /v1/simulations:rerunMetrics`.  **One-shot**  Both patch targets are write-once. Audio can be attached only once per conversation; a second PATCH on a conversation that already has audio returns `409 ALREADY_EXISTS`. Metadata is additive only; patching a key that already has a value returns `409 ALREADY_EXISTS`.  **Body**  Exactly one of `audio`, `audio_url` or `metadata` is required. 
     * @summary Attach audio or add metadata to a conversation
     * @param {string} conversationId Unique conversation identifier (22-26 characters)
     * @param {CovalConversationsAPIPatchConversationRequest} covalConversationsAPIPatchConversationRequest 
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
